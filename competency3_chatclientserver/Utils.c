			...
			if( (newBufferPtr = realloc(*inputStr, totalSize+1 )) != NULL ) {
				*inputStr = newBufferPtr;
				...
				if( (*inputStr)[strlen(*inputStr) - 1] == '\n') {
					...
					totalSize--;//new line was counted
				}
			}
			if( strlen(str) > 0 && str[strlen(str) - 1] != '\n') {
				Utils_flushStream(stdin);
...
int Utils_receiveDataFromSocket(int clientSockFd, char ** dynamicDataBuffer, int recvFlag) {
	...
	while( (bytesReceived = recv(clientSockFd, dataBuffer, PEEKQUANTITY, recvFlag)) > 0
			&& dataBuffer[0] != '\0' ) {

		nBytes += bytesReceived;

		//allocate memory of size nBytes+1, just incase input does not come with terminating null \0
		...
		if( bytesReceived < PEEKQUANTITY
				|| (bytesReceived == PEEKQUANTITY && dataBuffer[PEEKQUANTITY-1] == '\0'))  {
			break;
		}
	}
...
int Utils_doToSocket(EndSocketParams *params, SocketList *lSockets) {
	...
	if( strncmp(params->actionType, "close", 5) == 0 ) {
		retval = close( params->socketFd );
	} else if (strncmp(params->actionType, "shutdown", 5) == 0 ) {
		retval = shutdown(params->socketFd, params->shutdownHow );
	}
	if( retval == 0 ) {
		printf("Action '%s' was successfully done on Socket %d.\n", params->actionType, params->socketFd);
	} else {
		fprintf(stderr, "Issues with doing action '%s' on socket %d : %s (%d)\n",
				params->actionType, params->socketFd, strerror(errno), errno);
	}
	if( lSockets != NULL ) {
		UtilsLinkedList_removeFromListOfSockets(lSockets, params->socketFd);
	}
...
int Utils_doToAllSockets(SocketList *lSockets, EndSocketParams *params) {
	int count = 0;
	while ( !LIST_EMPTY(&lSockets->head) && (params->socketFd = lSockets->head.lh_first->socketFd ) > 0 ) {
		printf("\nDoing action '%s' on socket %d\n", params->actionType, params->socketFd);
		Utils_doToSocket( params, lSockets);
		count++;
	}
	return count;
}
...
void Utils_doToSocketsAndExit(EndSocketParams *params, SocketList *lSockets, char * errorMessage, int exitConst) {
	Utils_doToAllSockets(lSockets, params);
