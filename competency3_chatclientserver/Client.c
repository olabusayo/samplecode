...
int main(int argc, char *argv[]) {
    ...
    if( (retval = Utils_isValidInternetAddrStruct(serverAddr)) != VALID ) {
        ...
        Utils_doToSocketsAndExit(&params, &listOfOpenSockets, errorString, EXIT_FAILURE);
    }
    ...
    if( (retval = connect(serverSockFd, (struct sockaddr *) &serverAddr, sizeof(serverAddr) )) < 0 && retval != EISCONN) {
    	memset(errorString, '\0', sizeof(errorString));
    ...
    //spawn thread to receive from server
    //pthread_create()
    ...
    if( (nBytes = Utils_receiveDataFromSocket(serverSockFd, &dynamicDataBuffer, MSG_DONTWAIT) ) <= 0 ) {
    	memset(errorString, '\0', sizeof(errorString));
    ...
    Utils_doToAllSockets(&listOfOpenSockets, &params);
...
void Client_handleSignalInterrupt(int signum) {
	...
	Utils_doToSocketsAndExit(&params, &listOfOpenSockets, errMsg, EXIT_SUCCESS );
}
